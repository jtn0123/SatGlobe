...
| `ReportData.body` / `columns` / `isHeaders` | Externally registered reports still use the complete legacy string-body contract; the formatter converts those three fields into the structured table model. | Version the plugin report API, migrate all three fields together, and provide a migration period with legacy-format characterization tests. |
