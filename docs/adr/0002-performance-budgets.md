...
The measurements below are dated evidence for the code named in the context baseline, not current evidence for the later combined application. ADR 0004 governs fresh five-sample records and starts that application's accepted ledger empty.
...
| 6 | Conjunction lens | Deferred feed parse **<50 ms**, headed lens response p95 **≤100 ms**, headed idle median **≥59.8 fps** | Production E2E asserts the parse measure; `benchmark:satglobe` fails on the interaction and idle thresholds | Packet 4 adds no per-frame conjunction work; loading starts only after catalog-ready in an idle callback |
| 7 | Launch-history step | **One** combined filter/recolor/count pass per stop, headed synchronous apply p95 **≤100 ms**, paused median **≥59.8 fps**, and zero paused interaction measures | Production E2E asserts the exact pass count and monotonic catalog result; `benchmark:satglobe` fails synchronous apply, idle, or churn thresholds | Packet 7 measured 47.2 ms median / 51.3 ms p95 synchronous apply, 59.88 fps paused, and no paused churn at the full installed catalog |
| 8 | PNG snapshot export | **Zero** idle/per-frame retention cost; one PNG encode for one accepted user request; full canvas backing dimensions with non-blank decoded pixels | Unit tests enforce the one-slot/one-flush contract; production E2E checks configured and actual `preserveDrawingBuffer: false`, PNG/IHDR dimensions, browser decode, and pixel variance in Present and Story | Packet 8 reads only the requested completed frame immediately after the multi-viewport render and keeps no retained drawing buffer |
...
6. Packet 8 keeps SatGlobe's WebGL context at `preserveDrawingBuffer: false`. A single guarded request owns the renderer capture slot through asynchronous `toBlob` completion and is flushed once, directly after `ViewportManager.renderAll`; no timer, polling loop, retained bitmap, or routine readback exists when the user is not exporting.
...
- Snapshot export pays the browser's PNG encoding and readback cost only on explicit user action. Requests cannot overlap, and lifecycle failures release the slot instead of leaving a stuck busy state; the normal render path adds only a null-slot check after the completed frame.
