...
describe('getSunDirection_method', () => {
  it.each([
    [2451545.0, [26942821.97188163, -135008996.02114344, -58533563.41528564]],
    [2458850.5, [28629425.15938371, -134719134.74737248, -58400628.90499934]],
    [2459203.5, [-3760409.036826631, -137212823.5992045, -59481283.378439635]],
  ])('returns the expected sun direction for Julian date %d', (julianDate, expectedDirection) => {
    expect(SatMath.getSunDirection(julianDate)).toEqual(expectedDirection);
  });
...
describe('calculateVisMag_method', () => {
  it('matches the established visual-magnitude result for fixed geometry', () => {
    const sat = defaultSat.clone();

    sat.position = { x: -7000 as Ootk.Kilometers, y: 1000 as Ootk.Kilometers, z: 2000 as Ootk.Kilometers };
    sat.vmag = 5;
    const getRaeSpy = vi.spyOn(SatMath, 'getRae').mockReturnValue({ az: 0, el: 0, rng: 1500 } as never);
    const sun = new Sun();

    sun.eci = { x: DISTANCE_TO_SUN, y: DISTANCE_TO_SUN, z: DISTANCE_TO_SUN } as TemeVec3;

    expect(SatMath.calculateVisMag(sat, defaultSensor, new Date('2026-07-18T00:00:00Z'), sun)).toBeCloseTo(5.4092555242187075, 12);
    getRaeSpy.mockRestore();
  });

  it('returns the eclipsed sentinel when phase geometry has zero length', () => {
    const sat = defaultSat.clone();

    sat.position = { x: 0 as Ootk.Kilometers, y: 0 as Ootk.Kilometers, z: 0 as Ootk.Kilometers };
    const getRaeSpy = vi.spyOn(SatMath, 'getRae').mockReturnValue({ az: 0, el: 0, rng: 1500 } as never);
    const sun = new Sun();

    sun.eci = { x: DISTANCE_TO_SUN, y: DISTANCE_TO_SUN, z: DISTANCE_TO_SUN } as TemeVec3;

    expect(SatMath.calculateVisMag(sat, defaultSensor, new Date('2026-07-18T00:00:00Z'), sun)).toBe(30);
    getRaeSpy.mockRestore();
  });
